...
(define (do-draw cr widget)
  ...
  (let ([pattern1 (CAI:cairo-pattern-create-for-surface surface1)]
    ...
    (CAI:cairo-pattern-set-extend (CAI:cairo-get-source cr) 'reflect) ;;reflect none pad from  guile-cairo-enum-types.c
    ...
    (CAI:cairo-pattern-set-extend (CAI:cairo-get-source cr) 'pad)
    (CAI:cairo-rectangle       cr 20 140 100 100)
    (CAI:cairo-fill            cr)
  
    (CAI:cairo-set-source         cr pattern4)
    ;;(CAI:cairo-pattern-set-extend (CAI:cairo-get-source cr) 'none)
